...
// Flatten sidebar groups into reading order, descending into nested items and
// dropping stubs (comingSoon or slug-less) so navigation only ever points at
// real, live pages. A nested page keeps its top-level group title.
export function flattenNav(groups: DocNavGroup[]): FlatNavEntry[] {
  const out: FlatNavEntry[] = [];
  const walk = (items: DocNavItem[], groupTitle: string): void => {
    for (const item of items) {
      if (!item.comingSoon && item.slug !== undefined) {
        out.push({ label: item.label, slug: item.slug, groupTitle });
      }
      if (item.items) walk(item.items, groupTitle);
    }
  };
  for (const group of groups) walk(group.items, group.title);
  return out;
}
